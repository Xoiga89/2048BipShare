...
*/
// Này là bản cho đồng hồ ở Germany
#include "libbip.h"
#include "2048.h"
  ...
  if ( (param0 == *app_data_p) && get_var_menu_overlay()){
    ...
    *app_data_p = (struct app_data_ *)pvPortMalloc(sizeof(struct app_data_));
	...
	if ( settings.seconds_between_rec == 0) {
		app_data->seconds_between_rec = 30;
	}
	else {
	...
	app_data->weather_dem = 0;
		...
		time_change = app_data->seconds_between_rec * 1000;
					...
					if (app_data->menu_stage == 0){ //Bật đèn nền hay không
						...
						if ( get_left_side_menu_active() ) 		
						    app_data->proc->ret_f = show_watchface;
									...
									update_settings();
						// gọi hàm trả về (thường đây là menu khởi chạy), dưới dạng một tham số, chúng tôi chỉ định địa chỉ của hàm trong ứng dụng của chúng tôi
						if ((gest->touch_pos_x >= 100 && gest->touch_pos_x <= 176) && (gest->touch_pos_y >= 140 && gest->touch_pos_y <= 176)){
						show_menu_animate(app_data->ret_f, (unsigned int)show_screen, ANIMATE_RIGHT);
						} else {
						app_data->screen = 1;
...
int tim_cal(){
  ...
  load_font();
        set_fg_color(COLOR_WHITE);
		text_out_center("Đợi 15 giây", 80, 60); 
...
/*
int find_body_temp(){	//	функция поиска минимального значения в массиве
	struct app_data_ **app_data_p = get_ptr_temp_buf_2(); 
  struct app_data_ *app_data = *app_data_p;
...
if ( heartrate < 74 ) {
	...
	t = 375 / 100 + (heartrate - 86) / 8;
}	
	return t;	//	возвращаем минимальное значение
}
*/
/*
https://pubmed.ncbi.nlm.nih.gov/2424378/#:~:text=The%20mean%20heart%20rate%20during,remained%20high%2C%20even%20during%20sleep.
https://journals.lww.com/ccmjournal/Abstract/2021/03000/The_Relationship_Between_Heart_Rate_and_Body.30.aspx
...
if ( find_hr() < 74 ) {
	...
	t = 375 / 10 + (find_hr() - 86) / 8;
		...
		set_fg_color(COLOR_GREEN);
		text_out("Body Temp (C): ", 10, 140); //V02max
		char temp[10];
		_sprintf(temp, "%d", t);
		text_out(temp, 130, 140);
...
void menu(){	//	 функция рисования экранов настройки
	...
	if (app_data->menu_stage == 0){
		...
		char max[10];
		_sprintf(max, "%d", find_max() + deep_them / 60000);
		...
		text_out("Tong:", 10, 150); //Tong
			...
			set_bg_color(COLOR_BLACK);
		set_fg_color(COLOR_WHITE);
		_sprintf(sum, "%d", find_sum() + ngu_them / 60000);
...
int find_min(){	//	функция поиска минимального значения в массиве
	...
	if ( find_max() * 100 / find_sum() >= 46 ) {
		min = find_sum() * (100 - 41) / 100;
	} else {
...
int find_max(){	//	функция поиска минимального значения в массиве
	...
	int t = app_data->seconds_between_rec;
	//if(app_data->seconds_between_rec % 60 == 30 ){ t = t + 30; }
	...
	for (int i = 0; i < app_data->rec_counter ; i++){
		...
		if ( (app_data->records[i] - app_data->records[i - 1] < 2 ) && (app_data->records[i] - app_data->records[i - 1] > -3 ) && (app_data->records[i] < 68) ){
			a = a + t; 
			if (a >= 9 * 60) {
			max = max + a ;
			}
...
struct settings_ settings;
	...
	settings.backlight = app_data->backlight;
        ...
        settings.nhe = find_min();
		settings.deep = find_max() + deep_them / 60000;
		settings.sleep = find_sum() + ngu_them / 60000;
		settings.tinh = find_avg();
...
switch (app_data->status){	//	trạng thái cảm biến quá trình
	default:
	case 5:{		//	đóng băng chưa hoàn thành
		deep_them = deep_them + 200;
		ngu_them = ngu_them + 200;
		//deep_them = deep_them + 200; ngu_them = ngu_them + 200; 200 ở đây là bằng với 200 trong set_update_period(1, 200);
		set_update_period(1, 200);	//	заново заводим таймер
		time_change = 200;	//	khởi động lại bộ đếm thời gian
...
void thegioi1(){  // Lấy giờ Úc (Giờ DE +9) , VN (Giờ DE +6) , Los Angeles (Giờ DE -9) , London (Giờ DE -1). Ký tự nơi ở sẽ thấy kí tự theo Mã chung ISO và cách 15 px, Giờ + thì > 24, Giờ - thì < 0)
...
text_out_center("TONY", 88, 30);
  ...
  set_bg_color(COLOR_BLACK);
  text_out_center("GER: ", 60, 58);
  char clock_time_GE[8]; 			//	текст время		12:34
		struct datetime_ dt;
		get_current_date_time(&dt);
		_sprintf(clock_time_GE, "%02d:%02d", dt.hour, dt.min);
		//text_out_center(clock_time, 125, 163); // печатаем результат(время) большими цифрами
		text_out_font(11, clock_time_GE, 80 , 58, 1); // text_out_font(Kiểu font chữ, Char (Ở đây là Clock_time roi), x , y, Khoảng cách)
		repaint_screen_lines(0, 176); 
set_update_period(1, 60000);  
repaint_screen_lines(0, 176);     
  ...
  char clock_time_VN[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 10 không? Không thì sao: Đã làm
				 ...
				 //Xét tháng 3 trước
                  if (( dt.month == 3) && ( dt.day == VN3)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour + 6 - 1 > 24) {
									_sprintf(clock_time_VN, "%02d:%02d", dt.hour + 6 - 24 - 1, dt.min);
												}
								else {
									_sprintf(clock_time_VN, "%02d:%02d", dt.hour + 6 - 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){ 
								if (dt.hour + 6 > 24) {
									_sprintf(clock_time_VN, "%02d:%02d", dt.hour + 6 - 24, dt.min);
												}
								else {
									_sprintf(clock_time_VN, "%02d:%02d", dt.hour + 6, dt.min);
									}
										}
					}
					...
					//Còn lại không thuộc 2 trường hợp trên là (Trước ngày chủ nhật)
						if (( dt.month == 3) && ( dt.day < VN3)){
						 ...
						 // Dấu kết cho tháng 3
				//Xét đến tháng 10
                  if (( dt.month == 10) && ( dt.day == VN10)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					//Xét riêng trong ngày chủ nhật lúc 2h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 2){ // Giờ ở Đức từ 2h sáng chủ nhật là về lại bình thường
									if (dt.hour + 6 > 24) {
									_sprintf(clock_time_VN, "%02d:%02d", dt.hour + 6 - 24, dt.min);
												}
								else {
		...
		//text_out_center(clock_time, 125, 163); // печатаем результат(время) большими цифрами
		text_out_font(11, clock_time_VN, 38 , 104, 1); // text_out_font(Kiểu font chữ, Char (Ở đây là Clock_time roi), x , y, Khoảng cách)
		repaint_screen_lines(0, 176); 
set_update_period(1, 60000);  
repaint_screen_lines(0, 176);     
  ...
  char clock_time_AU[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 10 không? Không thì sao: Đã làm
			 //Xét tháng 3 trước
                  if (( dt.month == 3) && ( dt.day == AU3)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour + 9 - 1 > 24) {
									_sprintf(clock_time_AU, "%02d:%02d", dt.hour + 9 - 24 - 1, dt.min);
												}
								else {
									_sprintf(clock_time_AU, "%02d:%02d", dt.hour + 9 - 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){ 
								if (dt.hour + 9 > 24) {
									_sprintf(clock_time_AU, "%02d:%02d", dt.hour + 9 - 24, dt.min);
												}
								else {
									_sprintf(clock_time_AU, "%02d:%02d", dt.hour + 9, dt.min);
									}
										}
					}
					//Xét các ngày sau ngày chủ nhật trong tuần cuối đó (Bị thay đổi giống ngày chủ nhật: +1h)
...
repaint_screen_lines(0, 176);    
  ...
  for (int v = 25; v < 32 ; v++){
	...
	//Cho tháng 10
	 for (int b = 25; b < 32 ; b++){
	    ...
	    //Cho tháng 3 tuần thứ 2
  for (int n = 8; n < 15 ; n++){
		if ( (3 + CC + LY + YC + n)% 7 == 0 ){ //Số 3 ở đầu là đại diện cho tháng
			US32 = n ;
		} 
	}
	//Cho tháng 10
  ...
  char clock_time_US[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 11 không? Không thì sao: Đã làm
		    //-	Bắt đầu: Ngày chủ nhật thứ 2 của tháng 3 (8 =< x =< 8+6) lúc 2h sáng: Tăng thêm 1h --> Giờ VN -14
			//-	Kết thúc: Ngày chủ nhật thứ 1 của tháng  11 (1 =< x =< 7) lúc 2h
			 if (( dt.month == 3) && ( dt.day == US32)){
				 //Xét tháng 3 trước
                 
				//Xét trong tuần thứ 2 cho USA bị thay đổi trước? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 2h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 2){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour - 9 + 1 < 0) {
									_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 + 24 + 1, dt.min);
												}
								else {
									_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 + 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 2){
									if (dt.hour - 9 < 0) {
									_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 + 24, dt.min);
												}
								else {
									_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9, dt.min);
									}
								}
				}
					//Xét các ngày sau ngày chủ nhật trong tuần cuối đó (Bị thay đổi giống ngày chủ nhật: +1h)
					//Còn lại không thuộc 2 trường hợp trên là (Trước ngày chủ nhật)
						 if (( dt.month == 3) && ( dt.day < US32)){
							 if (dt.hour - 9 < 0) {
								...
								} //Hết xét tuần đầu tiên cho USA bị thay đổi trước
				//Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					...
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour - 9 - 1 + 1 < 0) {
									_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 + 24 - 1 + 1, dt.min);
												}
								else {
									_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 - 1 + 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){
					...
					//Còn lại không thuộc 2 trường hợp trên là (Trước ngày chủ nhật)
						 //Hết xét tuần cuối cùng
					if (( dt.month == 3) && ( dt.day < US3) && ( dt.day > US32)){
						if (dt.hour - 9 < 0) {
								_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 + 24 + 1, dt.min);
							}
							else {
						...
						} // Khoảng giữa 2 tuần thứ 2 đến tuần cuối tháng 3
					 // Dấu kết cho tháng 3
				//Xét đến tháng 10
                  if (( dt.month == 10) && ( dt.day == US10)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					...
					//Còn lại không thuộc 2 trường hợp trên là (Trước ngày chủ nhật)
						if (( dt.month == 10) && ( dt.day < US10)){
						 ...
						 // Dấu kết cho tháng 10
				//Xét đến tháng 11
                 if (( dt.month == 11) && ( dt.day == US11)){
					  //Xét trong tuần Đầu tiên cho USA thay đổi? Còn lại thì sao: Đã làm
					...
					//Còn lại không thuộc 2 trường hợp trên là (Trước ngày chủ nhật)
						 if (( dt.month == 11) && ( dt.day < US11)){
							 if (dt.hour - 9 + 1 < 0) {
								...
								} //Hết xét tuần đầu tiên cho USA
						 // Dấu kết cho tháng 11		
				//Xét các tháng còn lại trong khoảng từ tháng 3 đến tháng 11
					 if ( ( dt.month >= 4) && ( dt.month <= 9) ) { 
					 if (dt.hour - 9 - 1 + 1 < 0) {
							_sprintf(clock_time_US, "%02d:%02d", dt.hour - 9 + 24 - 1 + 1, dt.min);
						}
...
repaint_screen_lines(0, 176); 
  ...
  text_out_center("GBR: ", 110, 147); //Nước anh
      char clock_time_GB[8]; 			//	текст время		12:34
		 ...
		 if (dt.hour - 1 < 0) {
  _sprintf(clock_time_GB, "%02d:%02d", dt.hour - 1 + 24, dt.min);
 }
  else {
 _sprintf(clock_time_GB, "%02d:%02d", dt.hour - 1, dt.min);
  }
		//text_out_center(clock_time, 125, 163); // печатаем результат(время) большими цифрами
...
void thegioi2(){  // Lấy giờ Nga (Giờ DE +2) , Trung  (Giờ DE +7) , Nhật (Giờ DE +8) , Argentina (Giờ DE -4). Ký tự nơi ở sẽ thấy kí tự theo Mã chung ISO và cách 15 px, Giờ + thì > 24, Giờ - thì < 0)
  set_bg_color(COLOR_WHITE);
  fill_screen_bg();
  set_fg_color(COLOR_BLACK);
text_out_center("WORLD CLOCK", 88, 6);
...
repaint_screen_lines(0, 176);     
  ...
  char clock_time_RU[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 10 không? Không thì sao: Đã làm
			 //Xét tháng 3 trước
                  if (( dt.month == 3) && ( dt.day == RU3)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour + 2 - 1 > 24) {
									_sprintf(clock_time_RU, "%02d:%02d", dt.hour + 2 - 24 - 1, dt.min);
												}
								else {
									_sprintf(clock_time_RU, "%02d:%02d", dt.hour + 2 - 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){ 
								if (dt.hour + 2 > 24) {
									_sprintf(clock_time_RU, "%02d:%02d", dt.hour + 2 - 24, dt.min);
												}
								else {
									_sprintf(clock_time_RU, "%02d:%02d", dt.hour + 2, dt.min);
									}
										}
					}
					//Xét các ngày sau ngày chủ nhật trong tuần cuối đó (Bị thay đổi giống ngày chủ nhật: +1h)
...
repaint_screen_lines(0, 176);     
  ...
  char clock_time_JP[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 10 không? Không thì sao: Đã làm
			  //Xét tháng 3 trước
                  if (( dt.month == 3) && ( dt.day == JP3)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour + 8 - 1 > 24) {
									_sprintf(clock_time_JP, "%02d:%02d", dt.hour + 8 - 24 - 1, dt.min);
												}
								else {
									_sprintf(clock_time_JP, "%02d:%02d", dt.hour + 8 - 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){ 
								if (dt.hour + 8 > 24) {
									_sprintf(clock_time_JP, "%02d:%02d", dt.hour + 8 - 24, dt.min);
												}
								else {
									_sprintf(clock_time_JP, "%02d:%02d", dt.hour + 8, dt.min);
									}
										}
					}
					//Xét các ngày sau ngày chủ nhật trong tuần cuối đó (Bị thay đổi giống ngày chủ nhật: +1h)
...
repaint_screen_lines(0, 176);    
  ...
  char clock_time_CN[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 10 không? Không thì sao: Đã làm
			 //Xét tháng 3 trước
                  if (( dt.month == 3) && ( dt.day == CN3)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour + 7 - 1 > 24) {
									_sprintf(clock_time_CN, "%02d:%02d", dt.hour + 7 - 24 - 1, dt.min);
												}
								else {
									_sprintf(clock_time_CN, "%02d:%02d", dt.hour + 7 - 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){ 
								if (dt.hour + 7 > 24) {
									_sprintf(clock_time_CN, "%02d:%02d", dt.hour + 7 - 24, dt.min);
												}
								else {
									_sprintf(clock_time_CN, "%02d:%02d", dt.hour + 7, dt.min);
									}
										}
					}
					//Xét các ngày sau ngày chủ nhật trong tuần cuối đó (Bị thay đổi giống ngày chủ nhật: +1h)
...
repaint_screen_lines(0, 176); 
  ...
  char clock_time_AR[8]; 			//	текст время		12:34
           //Có phải trong tháng 3 đến tháng 10 không? Không thì sao: Đã làm
			  //Xét tháng 3 trước
                  if (( dt.month == 3) && ( dt.day == AR3)){
					  //Xét trong tuần cuối cùng? Còn lại thì sao: Đã làm
					 
					//Xét riêng trong ngày chủ nhật lúc 1h sáng mới thay đổi đồng hồ
								if ( dt.hour >= 1){ // Giờ ở Đức từ 1h sáng chủ nhật là cộng thêm 1h
									if (dt.hour - 4 - 1 < 0) {
									_sprintf(clock_time_AR, "%02d:%02d", dt.hour - 4 + 24 - 1, dt.min);
												}
								else {
									_sprintf(clock_time_AR, "%02d:%02d", dt.hour - 4 - 1, dt.min);
									}
								}
					//Còn lại là không bị thay đổi +1h
								if ( dt.hour < 1){ 
								if (dt.hour - 4 < 0) {
									_sprintf(clock_time_AR, "%02d:%02d", dt.hour - 4 + 24, dt.min);
												}
								else {
									_sprintf(clock_time_AR, "%02d:%02d", dt.hour - 4, dt.min);
									}
										}
					}
					//Xét các ngày sau ngày chủ nhật trong tuần cuối đó (Bị thay đổi giống ngày chủ nhật: +1h)
